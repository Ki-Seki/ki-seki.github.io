...
## Agentic Mode
When agentic mode is enabled, you can chain multiple steps to complete complex tasks. After each mutation, the system will use a separate LLM verification call to check if the user's request has been fully accomplished. If the verifier determines the task is complete, the loop stops; otherwise, you will be re-invoked with the updated DOM to continue. Focus on making one meaningful change per invocation.
...
**Agentic mode enabled by default:** Added a built-in agentic loop that allows the LLM to automatically chain multiple steps for complex tasks. After each mutation, the system makes a separate LLM verification call to check if the user's request has been fully accomplished. The verifier examines the updated DOM state and responds with YES (complete) or NO (continue). The system re-invokes the mutation LLM with the updated DOM, up to a configurable max of 5 iterations. A toggle in the terminal area lets users disable agentic mode for single-step behavior. The system prompt has been extended with an "Agentic Mode" section that explains the verification loop.
...
### v1.3 ([`a034f34`](https://github.com/Ki-Seki/ki-seki.github.io/commit/a034f34))
